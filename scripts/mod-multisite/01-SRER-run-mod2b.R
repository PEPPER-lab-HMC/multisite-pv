...
# Load data
pv <- read_csv("data_clean/03-pv-curve-data/SRER_pv_curve.csv")

# Quick plot
# Log y
pv |> 
  ggplot(aes(x = rwc_plotting,
             y = log(1/P.MPa))) +
...
# save(jm_coda, file = "scripts/mod-multisite/coda/coda_mod2b-SRER.Rdata")
...
# save(coda_rep, file = "scripts/mod-multisite/coda/rep_mod2b-SRER.Rdata")
...
summary(m1) # R2 = 0.9969, slope = 0.9937
...
ds <- param_sum |> 
  filter(grepl("^d", term)) |>
  mutate(ID = as.character(pv$ID)) |> 
  group_by(ID) |> 
  summarize(
    pred.mean = mean(pred.mean),
    std.error = mean(std.error),
    pred.lower = mean(pred.lower),
    pred.upper = mean(pred.upper)
  ) |> 
  ungroup() |> 
  mutate(Parameter = "d") |> 
  relocate(Parameter) |> 
...
#### Write out TLPs ####
write_csv(pred,
          file = "data_clean/JAGS-model-outputs/SRER-model-outputs/SRER_model_pred.csv")
write_csv(cps,
          file = "data_clean/JAGS-model-outputs/SRER-model-outputs/SRER_model_cps.csv")
write_csv(tlps,
          file = "data_clean/JAGS-model-outputs/SRER-model-outputs/SRER_model_tlps.csv")
write_csv(ds,
          file = "data_clean/JAGS-model-outputs/SRER-model-outputs/SRER_model_ds.csv")
write_csv(cs,
          file = "data_clean/JAGS-model-outputs/SRER-model-outputs/SRER_model_cs.csv")
