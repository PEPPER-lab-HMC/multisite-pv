...
pred <- read_csv("data_clean/JAGS-model-outputs/SRER-model-outputs/SRER_model_pred.csv") |> 
  select(-term)
cps <- read_csv("data_clean/JAGS-model-outputs/SRER-model-outputs/SRER_model_cps.csv") |>
  rename(cp_value = pred.mean, cp_std.error = std.error, cp_pred.lower = pred.lower, cp_pred.upper = pred.upper)
tlps <- read_csv("data_clean/JAGS-model-outputs/SRER-model-outputs/SRER_model_tlps.csv") |>
  rename(tlp_value = pred.mean, tlp_std.error = std.error, tlp_pred.lower = pred.lower, tlp_pred.upper = pred.upper, lab_x = x, lab_y = y)
ds <- read_csv("data_clean/JAGS-model-outputs/SRER-model-outputs/SRER_model_ds.csv") |> 
  rename(d_value = pred.mean, d_std.error = std.error, d_pred.lower = pred.lower, d_pred.upper = pred.upper)
cs <- read_csv("data_clean/JAGS-model-outputs/SRER-model-outputs/SRER_model_cs.csv") |> 
  rename(c_value = pred.mean, c_std.error = std.error, c_pred.lower = pred.lower, c_pred.upper = pred.upper)
...
pred <- pred |> left_join(pred_starts, join_by(ID))
